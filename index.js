...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:VARGAS_250\n' 
            + 'ORG: OWNER BOT;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=6281227560249:+62 812-2756-0249\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
const config = {
    XBOT: '❉VARGAS_250 BOT❉', 
    instagram: 'https://instagram.com/VARGAS_250/', 
    nomer: 'wa.me/6281227560249',
    youtube: 'https://youtube.com/channel/UCvVdgguxQxdSy8PI4GBLl1g', 
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, BOT BY VARGAS_250`)
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6281227560249@s.whatsapp.net"] 
          ...
          case 'ping':
            ...
            case 'kangcopet':
                ...
                case 'bug':
                    ...
                    client.sendMessage('6281227560249@s.whatsapp.net', options, text, {quoted: mek})
               ...
               if (!isGroup) return reply(mess.only.group)
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮ _*VARGAS_250*_ 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`})
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `❮ _*VARGAS_250*_ 𝘽𝙍𝙊𝘼𝘿𝘾𝘼𝙎𝙏 ❯\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('𝗴𝗮𝗴𝗮𝗹 𝗺𝗲𝗻𝗮𝗺𝗯𝗮𝗵𝗸𝗮𝗻 *target*, 𝗺𝘂𝗻𝗴𝗸𝗶𝗻 𝗸𝗮𝗿𝗲𝗻𝗮 𝗱𝗶 𝗽𝗿𝗶𝘃𝗮𝘁𝗲')
					}
					break
					case 'grup':
					case 'group':
       ...
       client.sendMessage(from, 'Tuh nomer owner ku >_<, kalau mau masukin aku ke grup chat dia ya dan jangan spam dia atau ku block kamu',MessageType.text, { quoted: mek} )
           break    
           case 'setname':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Succes, Mengganti Nama Grup', text, {quoted: mek})
                break
                case 'setdesc':
                if (!isGroup) return reply(mess.only.group)
			    if (!isGroupAdmins) return reply(mess.only.admin)
                ...
                client.sendMessage(from, 'Succes, Mengganti Deskripsi Grup', text, {quoted: mek})
                break
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
